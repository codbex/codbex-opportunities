angular.module('page', ['blimpKit', 'platformView', 'platformLocale']).controller('PageController', ($scope, ViewParameters, LocaleService) => {
	...
	$scope.filter = () => {
		...
		Dialogs.postMessage({ topic: 'codbex-opportunities.Settings.LeadQualificationTimeline.entitySearch', data: {
			entity: entity,
			filter: filter
		}});
		$scope.cancel();
	...
	$scope.cancel = () => {
		Dialogs.closeWindow({ id: 'LeadQualificationTimeline-filter' });
	};
